...
for _ in range(steps):
    print(_)
    # Prediction step
    y_pred_nn = model.predict(X_test_multiple)

    # Save the solution as a dataframe of pixels
    # For animation
    projected_prediction = pd.DataFrame(np.reshape(y_pred_nn, (int(vertical_pixels), int(horizontal_pixels))))
    ...
    array_predictions = np.append(array_predictions, y_pred_nn)
