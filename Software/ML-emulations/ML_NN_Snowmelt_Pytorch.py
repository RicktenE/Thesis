...
X_test_multiple = X_train_tensor[:(horizontal_pixels * vertical_pixels), :]
...
for _ in range(steps):
    ...
    # Prediction step
    with torch.no_grad():
        y_pred_nn = model(X_test_multiple)

    # Save the solution as a dataframe of pixels
    # For animation
    projected_prediction = pd.DataFrame(
        np.reshape(y_pred_nn.numpy(), (int(vertical_pixels), int(horizontal_pixels)))
    )
    ...
    array_predictions = np.append(array_predictions, y_pred_nn.numpy())

    # Create the new test set for next prediction
    new_state = data_prep.only_y_label(y_pred_nn.reshape(-1, 1),
                                       horizontal_pixels,
                                       vertical_pixels,
                                       multiplesteps=True,
                                       print_true=False)
    ...
    for __ in range(len(list_drivers_once)):
        ...
        new_state = data_prep.driver_as_feature(
            df=new_state,
    ...
    # Add variable rate as feature
    df = data_prep.VARIABLE_rate_as_feature(
        df=new_state,
